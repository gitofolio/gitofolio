...
- 인증을 요청한 클라이언트가 서버 일때.

인증을 요청한 클라이언트가 서버 일때, 클라이언트는 OAuth애플리케이션 로그인으로 사전에 발급받아놓은 Personal Access Token을 사용해 인증 합니다.

- 인증을 요청한 클라이언트가 브라우저 일때.

인증을 요청한 클라이언트가 브라우저 일때는, 클라이언트는 OAuth애플리케이션 로그인으로 받은 임시토큰을 통해서 JWT를 발급받고 이 후, JWT를 이용해 인증 합니다. 이 때, 자신의 인증과정이 유효한지 보증해주는 Personal Access Token이 필요합니다.

[[server-auth]]
=== Server
이 목차에는 인증을 요청한 클라이언트가 서버일때 취해야 할 인증과정이 기술되어있습니다.

[[server-auth-flow]]
==== Authenticate-flow
인증을 요청한 클라이언트가 서버일때, 인증의 전체 과정은 아래와 같습니다.
...
[[client-auth]]
=== Client
이 목차에는 인증을 요청한 클라이언트가 브라우저일때 취해야 할 인증과정이 기술되어있습니다.

[[client-auth-flow]]
==== Authenticate-flow
인증을 요청한 클라이언트가 브라우저일때, 인증의 전체 과정은 아래와 같습니다.
...
[[auth-do]]
=== 인증 필요 api에 접근하기
앞선 두가지 과정(서버 인증, 브라우저 인증)중 한가지를 완료했다면, 클라이언트는 인증이 필요한 api에 접근할 수 있습니다.
