...
[[user-put]]
=== 유저 수정하기
DB에 저장되어있는 유저 정보를 수정합니다.

DB에 저장되어있는 유저를 수정하기위해 PUT요청을 합니다.

GET,POST,DELETE요청은 유저의 이름을 기반으로 동작하지만, PUT메소드는 유저의 id값을 기반으로 동작하게끔 구현되어있습니다.

NOTE: id값은 깃허브 인증 고유값으로 임의로설정 불가능하며 변경또한 불가능합니다.

[[user-put-request]]
==== Request
요청시 생성되는 응답은 아래와 같습니다.
...
[[user-put-response]]
==== Representation
성공적으로 수정되었다면 아래와 같은 표현이 응답됩니다.

include::{snippets}/user/put/http-response.adoc[]

[[user-put-errorhandle1]]
==== NonExistUserException

[[user-put-errorhandle2]]
==== EditException
