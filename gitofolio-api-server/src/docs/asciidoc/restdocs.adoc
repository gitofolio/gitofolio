...
1. 클라이언트는 서버로 인증요청을 보냅니다. 이때, 인증에 사용할 application과 인증이 끝나고 임시토큰을 발급받을 url을 파라미터로 보내야합니다. -user

2. 서버는 클라이언트를 application에 해당하는 인증서버로 리다이렉트 시킵니다.

3. 클라이언트는 인증서버에 자신의 정보를 입력해 인증합니다. -user

4. 서버는 인증서버로 부터 넘겨받은 유저 정보를 분석하여 임시토큰을 발급후, 1번에서 입력한 url로 임시토큰을 전달합니다.

CAUTION: 임시토큰은 1회성 토큰이며, 5분동안만 유지됩니다.

5. 클라이언트는 받은 임시토큰을 /token로 보내 실제사용할 토큰을 얻습니다.-user

[[auth-do]]
=== Authenticate-detail

이 목차에는 인증을 위해 클라이언트가 해야하는 행동이 자세히 적혀있습니다.
...
----
GET: http://api.gitofolio.com/oauth?application={사용할 인증 서버}&redirect={임시토큰을 발급받을 url}
----

임시토큰을 발급받을 url은 스킴을 포함한 완전한 url이여야 합니다.
...
앞선 인증과정이 성공적으로 완료되었다면, <<auth-flow>> 목차 1번에서 설정한 redirect파라미터에 입력된 url의 파라미터로 임시토큰이 발급됩니다.

CAUTION: 앞서 적었듯이, 임시토큰은 1회성 토큰이며, 5분동안만 유지됩니다.

예시로, 클라이언트가 redirect파라미터를 아래와 같이 입력했다면,

----
GET: http://api.gitofolio.com/oauth?application=github&redirect=http://api.gitofolio.com
---- 
...
CAUTION: JWT토큰 인증 필요 자세한 내용은 로그인된 유저 가져오기 목차를 참고하세요.
...
[[IncorrectClaimException]]
=== IncorrectClaimException
서버에서 생산한 Claim과 요청 헤더의 Claim이 다를경우 발생합니다.
