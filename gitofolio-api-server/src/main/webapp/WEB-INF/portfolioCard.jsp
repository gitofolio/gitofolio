<%@ page language="java" contentType="image/svg+xml; charset=UTF-8" pageEncoding = "UTF-8"%>
 ...
 <svg width="${svgDTO.getViewBoxWidth()}" height="${svgDTO.getViewBoxHeight()}" viewBox="0 0 ${svgDTO.getViewBoxWidth()} ${svgDTO.getViewBoxHeight()}" fill="none" xmlns="http://www.w3.org/2000/svg"
    ...
    <g filter="url(#shadow)">
        <rect x="4" y="4" width="${svgDTO.getRectWidth()}" height="${svgDTO.getRectHeight()}" rx="5" fill="white"/>
    </g>
	 
    <text class="name" x="70" y="42"> ${svgDTO.getName()} </text>
	<text class ="bygit" x="70" y="60"> by gitofolio </text>
	 
    <g transform="translate(0, ${svgDTO.getArrowY()})">	
		<path class = "arrow"
    ...
    <circle class="user_image" cx="36.5" cy="36.5" r="22.5" />
    ...
    <defs>
        ...
        <pattern id="user_profile_img" patternContentUnits="objectBoundingBox" width="1" height="1">
			<image width="1" height="1" href="data:image/png;base64,${svgDTO.getBase64EncodedImage()}"/>
        </pattern>
